...
1. **"Port 1" / "Port 4"** — read as shorthand for a portion size or price
   tier, not spelled out anywhere on the sheet. Need the actual meaning
   (size in grams? a price band?) and the value for each tier.
2. **Corner annotations** — "AMUKAI"/"AMWARE"(?), `WOB`, `APP`, "basket 2",
...
3. Whether Category A ("Roti & Curry") and A3 ("Masala Roti Roll") are two
