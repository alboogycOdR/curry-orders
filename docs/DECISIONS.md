...
| D-34 | §22's ten milestones split into **Phase 1** (1–7, milestone 8 narrowed to daily controls only, milestone 10 narrowed to the help/policies copy blocks) and **Phase 2** (milestone 8's menu editor, milestone 9 in full, the rest of milestone 10 — reports, retention purge, backups, runbook, hardening, load/security passes). Phase 1 shipped and merged to `main`; Phase 2's work items are tracked in `docs/PHASE_2_PLAN.md` | A pilot needs a working order book, payment queue and kitchen/collection flow before it needs a menu-editing UI, an assisted-order desk or CSV reports — daily controls (M8's narrowed slice) already lets staff sell out a dish or close a slot without the full editor. Phase 2 is not optional polish: §22 itself ties milestone 10 to go-live checklist items 2–7, and §20.1's acceptance criteria include lookup/reorder (milestone 9) — both must still land before pilot, just after the order-taking core is proven |
| D-35 | The day/slot picker, line steppers and Continue button are on **`/basket/`** (`public:basket`), not on `/order/`. `/order/` is the menu-browsing screen (add to cart) only; `/basket/` is step 1 of the checkout flow (review cart + pick day/slot). Supersedes D-32's note that `/order/` was the single merged "menu + cart + slot" screen. The spec (§6.1) named the intermediate screen `/cart` — the URL is `/basket/` here, which matches the nav label ("Basket") the design handoff and base.html already use, and avoids the slug collision with any future §6.1 `/cart`-named API resource | Splitting the two concerns (browse + add vs review + pick slot) makes each screen simpler and lets the order screen drop all day/slot fetch machinery. The SPEC's `/cart` name was never user-visible; the nav label has always been "Basket" |
